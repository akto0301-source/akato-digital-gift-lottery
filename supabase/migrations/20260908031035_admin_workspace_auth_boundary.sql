...
using (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active));
...
using (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role in ('owner', 'order_intake')))
with check (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role in ('owner', 'order_intake')));
create policy "owners can delete orders"
on public.admin_work_orders for delete to authenticated
using (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role = 'owner'));
...
using (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role in ('owner', 'order_intake', 'maker', 'delivery')))
with check (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role in ('owner', 'order_intake', 'maker', 'delivery')));
...
using (assignee_user_id is null or assignee_user_id = (select auth.uid()) or exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role = 'owner'))
with check (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active));
...
using (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role = 'owner'))
with check (exists (select 1 from public.admin_workspace_staff s where s.user_id = (select auth.uid()) and s.active and s.role = 'owner'));
...
using (
  ...
  and exists (select 1 from public.admin_work_order_photos p where p.storage_path = name)
);
