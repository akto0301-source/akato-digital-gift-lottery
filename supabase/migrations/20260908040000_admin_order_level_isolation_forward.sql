...
drop policy if exists "staff can read their active membership" on public.admin_workspace_staff;
drop policy if exists "active staff can read their own order access" on public.admin_work_order_access;
drop policy if exists "active staff can read orders" on public.admin_work_orders;
drop policy if exists "intake staff can create owned orders" on public.admin_work_orders;
drop policy if exists "intake staff can update orders" on public.admin_work_orders;
drop policy if exists "owners can delete orders" on public.admin_work_orders;
drop policy if exists "active staff can read sources" on public.admin_work_order_sources;
drop policy if exists "intake staff can append sources" on public.admin_work_order_sources;
drop policy if exists "owners can delete sources" on public.admin_work_order_sources;
drop policy if exists "active staff can read items" on public.admin_work_order_items;
drop policy if exists "intake staff can create items" on public.admin_work_order_items;
drop policy if exists "assigned roles can update items" on public.admin_work_order_items;
drop policy if exists "owners can delete items" on public.admin_work_order_items;
drop policy if exists "active staff can read tasks" on public.admin_work_order_tasks;
drop policy if exists "intake staff can create tasks" on public.admin_work_order_tasks;
drop policy if exists "active staff can update own or unassigned tasks" on public.admin_work_order_tasks;
drop policy if exists "owners can delete tasks" on public.admin_work_order_tasks;
drop policy if exists "active staff can read photo metadata" on public.admin_work_order_photos;
drop policy if exists "photo staff can create private metadata" on public.admin_work_order_photos;
drop policy if exists "owners can update photo metadata" on public.admin_work_order_photos;
drop policy if exists "owners can delete photo metadata" on public.admin_work_order_photos;
drop policy if exists "photo staff can upload order objects" on storage.objects;
drop policy if exists "active staff can read registered order objects" on storage.objects;
drop policy if exists "uploaders and owners can delete order objects" on storage.objects;
...
revoke insert on table public.admin_work_orders from authenticated;
drop policy if exists "intake staff can create owned orders" on public.admin_work_orders;

create or replace function public.create_admin_work_order(
  new_order_number text,
  new_customer_name text default null,
  new_customer_contact text default null,
  new_delivery_date date default null
)
returns public.admin_work_orders
language plpgsql
security definer
set search_path = ''
as $$
declare
  ...
  created_order public.admin_work_orders;
begin
  if caller_id is null or not exists (
    ...
    where s.user_id = caller_id
      and s.active
      and s.role in ('owner', 'order_intake')
  ) then
    raise insufficient_privilege using message = 'Active owner or order_intake role required';
  end if;

  insert into public.admin_work_orders (
    order_number,
    customer_name,
    customer_contact,
    delivery_date,
  ...
  insert into public.admin_work_order_access (order_id, user_id, granted_by)
  values (created_order.id, caller_id, caller_id);

  return created_order;
end;
$$;

revoke all on function public.create_admin_work_order(text, text, text, date) from public, anon;
grant execute on function public.create_admin_work_order(text, text, text, date) to authenticated;
